...
class account_refunds_customer(report_sxw.rml_parse):
    _name = 'account.refunds.customer'
    def __init__(self, cr, uid, name, context):
        ...
        super(account_refunds_customer, self).__init__(cr, uid, name, context)
    ...
    def get_refunds(self,parameter,form):
        ...
        if tipo == 'customer':
            ...
            if parameter:
                #print '2'
                where = "WHERE ai.partner_id = "+str(parameter)+" AND ai.type in ('out_refund') AND ai.state in ('open','cancel','paid') "
        elif tipo == 'seller':
            #print '4'
            if parameter:
                #print '5'
                where = "WHERE ai.saleer_id = "+str(parameter)+" AND ai.type in ('out_refund') AND ai.state in ('open','cancel','paid') "
            else:
                where = "WHERE ai.type in ('out_refund') AND ai.state in ('open','cancel','paid') and ai.saleer_id is null "
        else:
            #print '6'
            where = "WHERE ai.type in ('out_refund') AND ai.state in ('open','cancel','paid') AND ai.id="+str(parameter)+" "
    ...
    def customers(self, form):
        ...
        where = "AND ai.type in ('out_refund') AND ai.state in ('open','cancel','paid') "
        case = 0
        if tipo=='customer':
            case = 1
            ...
            where = "WHERE ai.type in ('out_refund') AND ai.state in ('open','cancel','paid') " + filter
...
report_sxw.report_sxw('report.account_refunds_customer',
                      'account.invoice',
                      'addons/account_report_extend/report/account_refunds_customer.rml',
                      parser=account_refunds_customer,
                      header=False)
