...
class account_invoice_seller(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(account_invoice_seller, self).__init__(cr, uid, name, context=context)
    ...
    def vencidas(self, customer, seller, form):
        ##print "form", form
        ...
        self.cr.execute("""
                    ...
                    """,(date_to, customer,seller, date_start,date_to, date_to))
        #self.cr.execute(sql)
        res = self.cr.dictfetchall()
        ##print 'vencidas',res
        return res
    
    def por_vencer(self, customer, seller, form):
        ##print "form", form
        date_start = form.get('date_start', False)
        date_to = form.get('date', False)
        type = form.get('type', False)
        ...
        res = self.cr.dictfetchall()
        ##print 'por vencidas',res
        return res
        
    def seller_invoices(self,form):
        ##print "form", form
        type = form.get('type', False)
        date_start = form.get('date_start', False)
        date_to = form.get('date', False)
        ...
        where = "WHERE date_invoice BETWEEN '"+str(date_start)+"' and '"+str(date_to)+"' AND type = 'out_invoice' "\
                "AND reconciled is not True AND state = 'open' "#AND (date_due-'"+str(date_to)+"')< 0 OR (date_due-'"+str(date_to)+"') > 0"
        
        if type =='seller':
            sql = "SELECT ai.saleer_id AS seller,ai.partner_id as customer,rp.name as name  ,ru.name as vendedor,'1' as case "\
                   "FROM account_invoice AS ai "\
                   "JOIN res_users AS ru ON(ai.saleer_id = ru.id) "\
                   "JOIN res_partner AS rp ON(ai.partner_id = rp.id) "
            if seller:
                where = where + " AND ai.saleer_id="+str(seller)+" "
            else:
                where = where + " AND ai.saleer_id is not null " 
            group = "GROUP BY ai.saleer_id,ai.partner_id, rp.name, ru.name ORDER BY ru.name, rp.name"
    ...
    def invoices_vencidas(self, seller, customer, form):
        ##print "form", form
        date_start = form.get('date_start', False)
        date_to = form.get('date', False)
        ...
        if seller:
            if customer:
                self.cr.execute("""
                         ...
                         AND (date_due-%s) < 0
                         ORDER BY dias, factura
                        """,(date_to, seller, customer,date_start,date_to, date_to))
    ...
    def invoices_por_vencer(self, seller, customer, form):
        date_start = form.get('date_start', False)
        date_to = form.get('date', False)
...
report_sxw.report_sxw('report.seller_portfolio',
                      'account.invoice',
                      'addons/account_report_extend/report/account_invoice_portfolio_seller.rml',
                      parser=account_invoice_seller, header=False)
