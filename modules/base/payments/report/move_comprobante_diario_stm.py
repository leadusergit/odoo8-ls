...
def get_date(fecha):
    ...
    return obj_fecha.strftime("Quito, %d de %B del %Y")

class move_comprobante_diario_stm(report_sxw.rml_parse):
    _name = 'move.comprobante.diario.stm'
    _description = 'Formato para el ingreso de caja'
    
    def __init__(self, cr, uid, name, context):
        super(move_comprobante_diario_stm, self).__init__(cr, uid, name, context)
                ...
                if line.account_id.id not in account_bank: 
                    if line.debit:
                        val = {'code':line.account_id.code,
                               'fundsname':line.funds_certificate_id and line.funds_certificate_id.name.split('-')[0] or '',
                               'account':tools.ustr(line.account_id.name)[:30],
                               'project':line.preproject_id.abbreviation or line.analytic_account_id.abbreviation,
        ...
        if lines:
            for line in lines:
                ...
                if line.account_id.id in account_bank and line.credit:
                    ...
                    self.credit += line.credit or 0.00 - line.debit or 0.00
                    res.append(val1)
                else:
...
report_sxw.report_sxw('report.move.comprobante.diario.stm',
                      'account.bank.statement',
                      "addons/payments/report/move_comprobante_diario_stm.rml",
                      parser=move_comprobante_diario_stm, header=False)
