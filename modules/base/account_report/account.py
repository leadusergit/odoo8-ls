...
class account_report_history(osv.osv):
    ...
    def _calc_value(self, cr, uid, ids, name, args, context):
        ...
        for a in acc_report_id:
            period_val = self.pool.get('account.period').read(cr, uid, [a['period_id'][0]])[0]
            period_id = self.pool.get('account.period').search(cr, uid, [('date_start', '<=', period_val['date_start']),('fiscalyear_id', '=', period_val['fiscalyear_id'][0])])
            tmp_ids[a['id']] = self.pool.get('account.report.report').read(cr, uid, [a['tmp']], context={'periods': period_id})[0]['amount']
        return tmp_ids
