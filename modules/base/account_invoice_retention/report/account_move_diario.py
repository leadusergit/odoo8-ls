...
##############################################################################
#
#    Author :  OpenConsulting  <info@openconsulting.com.ec>
#
...
class account_move_diario(report_sxw.rml_parse):
    ...
    def get_date(self, mov):
        ...
        if invoices:
            invoice = invoice_obj.browse(self.cr, self.uid, invoices[0])
            res = str(time.strftime('%d-%m-%Y', time.strptime(invoice.date_invoice, '%Y-%m-%d'))).upper()
        else:
            res = str(time.strftime('%d-%m-%Y', time.strptime(mov.date, '%Y-%m-%d'))).upper()
        return res
    
    def get_date_vence(self, mov):
        invoice_obj = pooler.get_pool(self.cr.dbname).get('account.invoice')
        res = ''
        invoices = invoice_obj.search(self.cr, self.uid, [('move_id', '=', mov.id)])
        if invoices:
            invoice = invoice_obj.browse(self.cr, self.uid, invoices[0])
            res = str(time.strftime('%d-%m-%Y', time.strptime(invoice.date_due, '%Y-%m-%d'))).upper()
        else:
            res = str(time.strftime('%d-%m-%Y', time.strptime(mov.date, '%Y-%m-%d'))).upper()
        return res
    ...
    def __init__(self, cr, uid, name, context):
        super(account_move_diario, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
                ...
                'invoice': self._info_invoice,
                'get_beneficiario':self.get_beneficiario,
                'get_date':self.get_date,
                'get_date_vence':self.get_date_vence,
                'get_detalle':self.get_detalle,
                'fundsname': lambda name: name and name.split('-')[0] or '',
                
                })

report_sxw.report_sxw('report.account.move.diario', 'account.move', 'addons/account_invoice_retention/report/account_move_diario.rml', parser=account_move_diario)
