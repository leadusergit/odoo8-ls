...
class account_voucher(osv.osv):
    ...
    def onchange_partner_id2(self, cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=None):
        ...
        if  mes==mes1:
            ...
            vals4 = self.recompute_voucher_lines2(cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=ctx)
            vals5 = self.recompute_payment_rate(cr, uid, ids, vals4, currency_id, date, ttype, journal_id, amount, context=context)      

            for key in vals4.keys():
                res[key].update(vals4[key])
            for key in vals5.keys():
                res[key].update(vals5[key])               
    ...
    def recompute_voucher_lines2(self, cr, uid, ids, partner_id, journal_id, price, currency_id, ttype, date, context=None):
        """
        ...
        for line in account_move_lines:
            ...
            #if fr== True and 'Fondos'in valores or 'Reserva'in valores :#and str(line.date)[:4]== int(str(date)[:4])-1: #1
            if str(date)[5:7] == str(line.date)[5:7] and 'Fondos'in valores or 'Reserva'in valores:
                print"str(date)[5:7]=%s"%str(date)[5:7] 
                print"str(line.date)[5:7]=%s"%str(line.date)[5:7]
                if _remove_noise_in_o2m():
                    continue
    ...
    def onchange_line_ids2(self, cr, uid, ids, line_dr_ids, line_cr_ids, amount, voucher_currency, type, context=None):
    ...
    def onchange_amount2(self, cr, uid, ids, amount, rate, partner_id, journal_id, currency_id, ttype, date, payment_rate_currency_id, company_id, context=None):
        if context is None:
            context = {}
        ctx = context.copy()
        ctx.update({'date': date})
        ...
        res = self.recompute_voucher_lines2(cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=ctx)
    ...
    def onchange_journal2(self, cr, uid, ids, journal_id, line_ids, tax_id, partner_id, date, amount, ttype, company_id, context=None):
        ...
        if context.get('payment_expected_currency') and currency_id != context.get('payment_expected_currency'):
            ...
            amount = 0
        if partner_id:
            res = self.onchange_partner_id2(cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context)
            for key in res.keys():
