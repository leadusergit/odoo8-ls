...
class hr_employee_expenses_adjustments(models.Model):
...
class hr_fiscalyear(models.Model):
    ...
    arte_max = fields.Float('Arte y Cultura', required=True, default=0.325, digits=(8,4))
    arte_max_amount = fields.Float('Monto máximo de ArteyCultura', compute='max_amount_arte')
    ...
    @api.one
    @api.depends('basic_amount', 'arte_max')
    def max_amount_arte(self):
        self.arte_max_amount = self.basic_amount * self.arte_max
...
class hr_employee_personal_expenses(models.Model):
    ...
    max_education = fields.Float('Máximo de Educacion', related='fiscalyear_id.education_max_amount')
    food = fields.Float('Alimentación', states={'validated': [('readonly', True)]})
    max_food = fields.Float('Máximo de Alimentacion', related='fiscalyear_id.food_max_amount')
    clothing = fields.Float('Vestimenta', states={'validated': [('readonly', True)]})
    max_clothing = fields.Float('Máximo de Vestimenta', related='fiscalyear_id.clothing_max_amount')
    health = fields.Float('Salud', states={'validated': [('readonly', True)]})
    max_health = fields.Float('Máximo de Salud', related='fiscalyear_id.health_max_amount')
    arte = fields.Float('Arte y Cultura', states={'validated': [('readonly', True)]})
    max_arte = fields.Float('Máximo de ArteyCultura', related='fiscalyear_id.arte_max_amount')
    ...
    #===========================================================================
    _sql_constraints = [
        ('uniq_declaration', 'unique(contract_id,fiscalyear_id)', 'Su formulario de impuesto a la renta ya fue declarado!')
    ]
    ...
    def get_values(self):
        ...
        date_start = self.fiscalyear_id.date_start
        
        fiscalyear = self.env['account.fiscalyear'].sudo().search([('date_start', '=', date_start)]).id 
        period_ids = self.env['account.period'].sudo().search([('fiscalyear_id', '=', fiscalyear)]).ids    
        payslip_ids= self.env['hr.payslip'].sudo().search([('employee_id', '=', self.employee_id.id),
                                                           ('period_id', 'in', period_ids),                                                        
                                                           ('state', 'not in', ['draft'])],
                                                           order='date_to desc', limit=12)
        
        income_ids = self.env['hr.income'].sudo().search([('employee_id', '=', self.employee_id.id),
                                                          ('period_id', 'in', periodos_ids)])
                                                          #('period_id.fiscalyear_id', '=', self.fiscalyear_id.id)],
                                                          #order='create_date desc', limit=1)
                                                 
        print" payslip_ids=%s"%payslip_ids
        print" income_ids =%s"%income_ids 
        ##base_renta11= sum([income_ir.value for income_ir in self.env['hr.income'].browse(income_ids) if income_ir.adm_id.impuesto_renta])
        
        for payslip in payslip_ids:                                                            
            period_code=self.env['account.period'].sudo().search([('id', '=', payslip.period_id.id)]).code
            periodo=self.env['hr.contract.period'].sudo().search([('code', '=', period_code)])                        
            excl_irenta1 = sum([expense.amount for expense in payslip.input_line_ids if expense.expense_type_id.impuesto_renta ])
            excl_irenta  = excl_irenta1 + sum([psl.amount for psl in payslip.line_ids if 'IESSPERSONAL' in psl.code and psl.slip_id in payslip])
            
            ##base_renta1  = sum([income.value for income in income_ids if income.adm_id.impuesto_renta])            
            base_renta1 = sum([incomes.amount for incomes in payslip.input_line_ids if incomes.adm_id.impuesto_renta])
            base_renta2 = sum([expense.amount for expense in payslip.input_line_ids if expense.expense_type_id.impuesto_renta])
            base_renta  = sum([psl.amount for psl in payslip.line_ids if psl.code =='BASIC' and psl.slip_id in payslip]) - base_renta2            
            ir_cobrado  = sum([expense.amount for expense in payslip.line_ids if expense.code =='IMPRENTA' and psl.slip_id in payslip])

            print"base_renta=%s"%base_renta
            print"excl_irenta=%s"%excl_irenta
            print"ir_cobrado=%s"%ir_cobrado
            self.aux1 += base_renta
            self.aux2 += base_renta1
            self.aux += excl_irenta

            """acumula descuentos de aporte personal iess y egresos con IR""" 

            print"self.aux1=%s"%self.aux1
            """acumula salarios,ingresos con IR,descuenta egresos con IR"""
            self.wage = self.aux1 + self.aux2
            """Acumula IR recaudado"""
            self.collected_amount += ir_cobrado#recaudo          
            
            print"self.wage=%s"%self.wage
            print"self.collected_amount=%s"%self.collected_amount
            
            """Parametro para método base_imponible_IR se aplica cuando salario es constante (no se usan los valores devueltos)"""
            base_renta_mir = base_renta1 + sum([psl.amount for psl in payslip.line_ids if psl.code =='BASIC' and psl.slip_id in payslip])             
            """"""
            base_imponible, recaudo = self.env['hr.expense'].sudo().base_imponible_IR(contract=self.contract_id,
                                                                                      period=payslip.period_id,
                                                                                      base_renta=base_renta_mir,
                                                                                      excl_irenta=excl_irenta)
            
            """Se aplica si salario es constante"""
            #self.wage =base_imponible + self.aux  
    ...
    @api.one
    @api.depends('incomes_amount', 'expenses_amount','aux')
    def _get_base_amount(self):
        self.base_amount = self.incomes_amount - self.aux - self.expenses_amount
...
class hr_expense(models.Model):
    ...
    def base_imponible_IR(self, cr, uid, contract, period, base_renta, excl_irenta):
        ...
        """print"PERIOD IR=%s"%period
        print"PERIOD ID IR=%s"%period.id
        print"PERIOD CODE IR=%s"%period.code
        print"base_rentaIR=%s"%base_renta
        print"excl_irentaIR=%s"%excl_irenta"""
        ir_cobrado = 0.00
        ...
        payslip_ids= self.pool['hr.payslip'].search(cr,uid, [('employee_id', '=', contract.employee_id.id),
                                                             ('period_id', '=', period.id),                                                        
                                                             ('state', 'not in', ['draft'])],
                                                              order='date_to desc', limit=12)
        #print"payslip_ids IR=%s"%payslip_ids
        
        
        income_ids = self.pool['hr.income'].search(cr, uid, [('employee_id', '=', contract.employee_id.id)])#,('period_id', '=', period.id)])

        # Sumo y resto los valores imponibles de los anteriores roles
        for payslip in self.pool['hr.payslip'].browse(cr, uid, payslip_ids):
            base_renta += sum([income.value for income in self.pool.get('hr.income').browse(cr ,uid, income_ids)  if income.adm_id.impuesto_renta and income.period_id.code==period.code])
            #base_renta += sum([psl.amount for psl in payslip.line_ids if psl.code =='BASIC'])            
            base_renta -= sum([expense.amount for expense in payslip.input_line_ids if expense.expense_type_id.impuesto_renta])
            #base_renta -= sum([expense.amount for expense in payslip.line_ids if expense.code =='IESSPERSONAL 11.45'])    
            ir_cobrado += sum([expense.amount for expense in payslip.line_ids if expense.code =='IMPRENTA'])
            #print"base_renta4 IR=%s"%base_renta
            #print"ir_cobrado IR=%s"%ir_cobrado
        ...
        #print"base_imponible=%s"%base_renta
        return base_renta, ir_cobrado
    
    def calcular_impuesto_renta(self, cr, uid, payslip, **args):
        ...
        personal_expense = pe_env.search(cr, uid, [('employee_id', '=', payslip.employee_id.id),
                                                   ('contract_id', '=', payslip.contract_id.id),
                                                   ('fiscalyear_id', '=', payslip.period_id.fiscalyear_id.id)],
                                         order='create_date desc', limit=1)
        ...
        BASE_IMPONIBLE, IR_COBRADO = self.base_imponible_IR(cr, uid,payslip.contract_id,payslip.period_id, base_renta, excl_irenta)
        BASE_IMPONIBLE -= expense_value
        #=======================================================================
        tax = pe_env.get_tax_id(payslip.period_id.fiscalyear_id, BASE_IMPONIBLE)        
        values = pe_env.get_table_amounts(tax, BASE_IMPONIBLE,payslip.period_id.fiscalyear_id)
        MONTH = 13 - DATE(payslip.period_id.date_start).month
