...
class PaymentOrder(models.Model):
    ...
    voucher_ids = fields.Many2many('account.voucher', string='Vouchers', readonly=True)    
    vchr = fields.Boolean(string='vchr', default=False , help='Al menos un voucher vinculado')
    ...
    def get_lines_by_partner(self, order):
        ...
        if order.state != 'open':
            raise Warning(
                _("Payment order %s is not in 'open' state")
                % order.reference)
