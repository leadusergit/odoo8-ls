...
class laundry_make_invoice(osv.osv_memory):
    _name = "laundry.make.invoice"
    _description = "Facturas Lavanderia"
    _columns = {
        'grouped': fields.boolean('Agrupar facturas', help='Las ordenes de servicio deben pertenecer al mismo cliente'),
    ...
    def view_init(self, cr, uid, fields_list, context=None):
        ...
        order = self.pool.get('laundry.management').browse(cr, uid, record_id, context=context)

        return False

    def make_invoices(self, cr, uid, ids, context=None):
        order_obj = self.pool.get('laundry.management')
        ...
        for laundry_order in order_obj.browse(cr, uid, context.get(('active_ids'), []), context=context):
            if laundry_order.invoice_ref_id:
                raise ValidationError(u'Orden de Servicio %s ya facturada'%laundry_order.tag_asignado)
            else:
                continue
